...
End Function
    ...
    'Get all of the operations for a part, not just the machining operations
Function GetAllPartOps(partNum As String, rev As String) As Variant()
    On Error GoTo AllPartOpsErr
    Set fso = New FileSystemObject
    params = Array("po.PartNum," & partNum, "po.RevisionNum," & rev)
    query = "SELECT po.OprSeq, po.OpCode FROM EpicorLive11.dbo.PartOpr po WHERE po.PartNum = ? AND po.RevisionNum = ?"

    Call ExecQuery(query:=query, params:=params, conn_enum:=Connections.E10)
    
    GetAllPartOps = sqlRecordSet.GetRows()
       
    Exit Function
    
AllPartOpsErr:
   'PartNum / Rev doesn't exist, someone may have massaged the revision at the JobEntry form
    Err.Raise Number:=Err.Number, Description:="Func: E10-GetAllPartOps" & vbCrLf & Err.Description
