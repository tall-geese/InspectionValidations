...
WHERE jo.JobNum = 'NV15283' AND jo.OpCode IN ('SWISS', 'CNC')
...
INNER JOIN EpicorLive10.dbo.LaborDtl ldt ON jo.OprSeq = ldt.OprSeq AND ldt.JobNum = jo.JobNum
LEFT OUTER JOIN EpicorLive10.dbo.EmpBasic emp ON ldt.EmployeeNum = emp.EmpID 
WHERE jo.JobNum = 'NV15035' AND jo.OpCode IN ('SWISS', 'CNC') AND (ldt.LaborQty + ldt.ScrapQty + ldt.DiscrepQty) > 0 AND ldt.LaborNote <> 'Adjustment' AND ldt.LaborNote NOT LIKE ('%DMR%')
